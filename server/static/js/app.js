...
function updateCPUChart(thing) {
    // console.log(thing)
    $('#cpu_activity .progress-bar').animate({width: thing.cpu + "%"}, 250);
...
function updateRAMChart(thing) {
    // console.log(thing)
    $('#ram_pressure .progress-bar').animate({width: thing.ram + "%"}, 250);
...
function updateDiskChart(thing) {
    ...
    $('#disk_consumed .progress-bar').animate({width: thing.disk + "%"}, 250);
